...
var (
	...
	buildDebugOutput = os.Getenv("XKENGINE_DEBUG") == "1"
	buildFlags       = os.Getenv("XKENGINE_GO_BUILD_FLAGS")
	modFlags         = os.Getenv("XKENGINE_GO_MOD_FLAGS")
)
...
func getKengineOutputFile() string {
	f := "." + string(filepath.Separator) + "kengine"
...
func setcapIfRequested(output string) error {
	if os.Getenv("XKENGINE_SETCAP") != "1" {
		return nil
	}
	...
	skipSudo := sudoNotFound != nil || os.Getenv("XKENGINE_SUDO") == "0"
...
func parseGoListJson(out []byte) (currentModule, moduleDir string, replacements []xkengine.Replace, err error) {
	...
	for {
		...
		if mod.Main {
			...
			moduleDir = mod.Dir
			replacements = append(replacements, xkengine.NewReplace(currentModule, moduleDir))
			continue
		}
		...
		replacements = append(replacements, xkengine.NewReplace(mod.Path, dst))
	}
	for _, idx := range unjoinedReplaces {
		...
		replacements[idx].New = xkengine.ReplacementPath(resolved)
	}
	return
}
...
// xkengineVersion returns a detailed version string, if available.
func xkengineVersion() string {
	mod := goModule()
	ver := mod.Version
	if mod.Sum != "" {
		ver += " " + mod.Sum
...
func goModule() *debug.Module {
	...
	if ok {
		...
		// The recommended way to build xkengine involves
		// creating a separate main module, which
		// TODO: track related Go issue: https://github.com/golang/go/issues/29228
		// once that issue is fixed, we should just be able to use bi.Main... hopefully.
		for _, dep := range bi.Deps {
			if dep.Path == "github.com/khulnasoft/xkengine" {
				return dep
			}
		}
		return &bi.Main

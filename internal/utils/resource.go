...
// for embedding into the Kengine binary.
...
func WindowsResource(version, outputFile, tempDir string) error {
	...
	// StringFileInfo
	vi.StringFileInfo.CompanyName = "https://khulnasoft.com/"
	vi.StringFileInfo.ProductName = "Kengine"
	vi.StringFileInfo.FileDescription = "Kengine"
	vi.StringFileInfo.InternalName = strings.TrimSuffix(basename, ext)
	vi.StringFileInfo.OriginalFilename = basename
	vi.StringFileInfo.LegalCopyright = "The Kengine Authors"
	...
	ico, err := embedFS.ReadFile("resources/ico/kengine.ico")
	if err != nil {
		return err
	}
	icoCopy, err := os.Create(filepath.Join(tempDir, "kengine.ico"))
	if err != nil {
		return err
	}
	// set ico path

...
// Builder can produce a custom Kengine build with the
...
// Build builds Kengine at the configured version with the
...
func (b Builder) Build(ctx context.Context, outputFile string) error {
	...
	if b.OS == "windows" {
		...
		cmd := buildEnv.newGoBuildCommand(ctx, "list", "-m", buildEnv.kengineModulePath)
		...
		// output looks like: github.com/khulnasoft/kengine/v2 v2.7.6
		version := strings.TrimSpace(strings.TrimPrefix(buffer.String(), buildEnv.kengineModulePath))
	...
	log.Println("[INFO] Building Kengine")
...
func newTempFolder() (string, error) {
	...
	if runtime.GOOS == "darwin" {
		// After upgrading to macOS High Sierra, Kengine builds mysteriously
		...
		// See: https://github.com/khulnasoft/kengine/issues/2036
		// and https://twitter.com/nxpkg6/status/978345803365273600 (thread)
...
const (
	...
	defaultKengineModulePath = "github.com/khulnasoft/kengine"
)

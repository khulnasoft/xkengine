...
func (b Builder) newEnvironment(ctx context.Context) (*environment, error) {
	// assume Kengine v2 if no semantic version is provided
	kengineModulePath := defaultKengineModulePath
	if !strings.HasPrefix(b.KengineVersion, "v") || !strings.Contains(b.KengineVersion, ".") {
		kengineModulePath += "/v2"
	}
	kengineModulePath, err := versionedModulePath(kengineModulePath, b.KengineVersion)
	if err != nil {
		return nil, err
	}
	...
	tplCtx := goModTemplateContext{
		KengineModule: kengineModulePath,
	}
	...
	cmd.Args = append(cmd.Args, "kengine")
	...
	// pin versions by populating go.mod, first for Kengine itself and then plugins
	log.Println("[INFO] Pinning versions")
	err = env.execGoGet(ctx, kengineModulePath, env.kengineVersion, "", "")
	if err != nil {
		return nil, err
	}
nextPlugin:
	for _, p := range b.Plugins {
		...
		// also pass the Kengine version to prevent it from being upgraded
		err = env.execGoGet(ctx, p.PackagePath, p.Version, kengineModulePath, env.kengineVersion)
		if err != nil {
			return nil, err
		}
		// check for early abort
	...
	// see https://github.com/khulnasoft/xkengine/pull/92
...
// created command will also have the value of `XKENGINE_GO_BUILD_FLAGS` appended to its arguments, if set.
...
// created command will also have the value of `XKENGINE_GO_MOD_FLAGS` appended to its arguments, if set.
...
// Kengine module/version we're building against; this will prevent the
// plugin module from causing the Kengine version to upgrade, if the plugin
// version requires a newer version of Kengine.
// See https://github.com/khulnasoft/xkengine/issues/54
func (env environment) execGoGet(ctx context.Context, modulePath, moduleVersion, kengineModulePath, kengineVersion string) error {
	mod := modulePath
	if moduleVersion != "" {
		mod += "@" + moduleVersion
	}
	kengine := kengineModulePath
	if kengineVersion != "" {
		kengine += "@" + kengineVersion
	}
	...
	if kengine != "" {
		cmd.Args = append(cmd.Args, mod, kengine)
	} else {
...
import (
	kenginecmd "{{.KengineModule}}/cmd"

	// plug in Kengine modules here
	_ "{{.KengineModule}}/modules/standard"
	{{- range .Plugins}}
	_ "{{.}}"
	{{- end}}
)
...
// originally published in: https://github.com/nxpkg/kengine-embed
...
import (
	...
	"{{.KengineModule}}"
	"{{.KengineModule}}/kengineconfig/kenginefile"
)
...
// FS implements a Kengine module and fs.FS for an embedded
// file system provided by an unexported package variable.
//
// To use, simply put your files in a subfolder called
// "files", then build Kengine with your local copy of this
...
func (FS) KengineModule() kengine.ModuleInfo {
	return kengine.ModuleInfo{
		ID:  "kengine.fs.embedded",
		New: func() kengine.Module { return new(FS) },
	}
}
...
// UnmarshalKenginefile exists so this module can be used in
// the Kenginefile, but there is nothing to unmarshal.
func (FS) UnmarshalKenginefile(d *kenginefile.Dispenser) error { return nil }

// Interface guards
var (
	_ fs.FS                 = (*FS)(nil)
	_ kenginefile.Unmarshaler = (*FS)(nil)
)
`
